...
describe('Commands', () => {
  ...
  describe('insertResource', () => {
    ...
    it('returns a function', () => {
      jest.resetModules()
      const insertResource = require('../../../commands/insert-resource').default
    ...
    describe('when the current file mode is JSON', () => {
      ...
      it('inserts a JSON template and selects the logical ID', async () => {
        ...
        expect(vscodeWorkspaceGetConfigurationGetSpy).toHaveBeenCalledWith('editor.tabSize', 2)
        ...
        expect(context.workspaceState.get).toHaveBeenCalledWith('publicTypeListCache', {updatedAt: 0, typeSummaries: []})
